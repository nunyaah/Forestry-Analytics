"""
Canopy Cover Model Training
============================
Trains a Random Forest regression model to predict Hansen treecover2000 (0-100%)
from Landsat 5/7 reflectance + terrain predictors.

Usage:
    python canopy_cover/train_model.py

Output:
    canopy_cover/models/canopy_cover_rf.joblib   — trained model
    canopy_cover/models/canopy_cover_rf_metrics.json — evaluation metrics
"""
...
DATA_CSV    = os.path.join(BASE_DIR, "data", "canopy_cover_training.csv")
MODEL_DIR   = os.path.join(BASE_DIR, "models")
MODEL_PATH  = os.path.join(MODEL_DIR, "canopy_cover_rf.joblib")
METRICS_PATH = os.path.join(MODEL_DIR, "canopy_cover_rf_metrics.json")
...
def load_and_split(csv_path: str):
    ...
    # Drop any remaining null values in feature/target columns
    before = len(df)
    ...
    # Split using the pre-generated random 'split' column
...
def train():
    log.info("=" * 60)
    log.info("Canopy Cover Model Training")
    log.info("=" * 60)
    ...
    log.info("  OOB R² (train set): %.4f", rf.oob_score_)
    ...
    # ------------------------------------------------------------------
    # Permutation importance on validation set (more reliable than MDI)
    # ------------------------------------------------------------------
